...
// PSM 11 = SPARSE_TEXT — finds text anywhere on the card without assuming layout
const PSM_SPARSE_TEXT = '11';

// Scale up small images and boost contrast so Tesseract reads thin card text better
async function preprocessForOCR(imageFile) {
  ...
  // Grayscale + contrast boost using the standard factor formula
  const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  const d = imageData.data;
  const contrast = 45; // positive = more contrast
...
export async function recognizeCard(imageFile, onProgress) {
  ...
  return data; // contains data.lines (with confidence), data.text, data.words
}

// Clean a line: collapse whitespace, strip leading/trailing punctuation noise
function cleanLine(str) {
  return str.replace(/\s{2,}/g, ' ').replace(/^[^a-zA-Z฀-๿\d]+|[^a-zA-Z฀-๿\d.]+$/g, '').trim();
}

export function parseCardFields(data) {
  // Use Tesseract line objects filtered by confidence (0–100)
  // Fall back to splitting raw text if line data unavailable
  let lines;
  ...
  // Also build a flat string for multi-line regex patterns
  const text = lines.join('\n');
  const result = {};
  const used = new Set(); // line indices already claimed
  ...
  // ── WEBSITE ────────────────────────────────────────────────────────────────
  // Match www.xxx or http(s)://xxx but never an email address
  ...
  // ── PHONE (with label-first strategy) ─────────────────────────────────────
  // 1. Prefer labeled lines: Tel / Mobile / Phone / Fax / HP
  const telLabelPat = /(?:^|\s)(?:Tel|Tél|Phone|Mobile|Mob|T|HP)[\s.:]/i;
  const faxLabelPat = /(?:^|\s)(?:Fax|F)[\s.:]/i;

  const telLines = lines.map((l, i) => ({ l, i })).filter(({ l, i }) =>
    !used.has(i) && telLabelPat.test(l) && !faxLabelPat.test(l),
  );
  const faxLines = lines.map((l, i) => ({ l, i })).filter(({ l, i }) =>
    !used.has(i) && faxLabelPat.test(l),
  );

  const extractNums = arr =>
    arr.flatMap(({ l, i }) => {
      used.add(i);
      return [...l.matchAll(/\+?[0-9][\d\s\-()+.]{5,}/g)].map(m => m[0].trim().replace(/\s+/g, ' '));
    });

  const telNums = extractNums(telLines);
  if (telNums.length) result.phone = telNums.join(' / ');

  // Mark fax lines used even if we don't store them as phone
  faxLines.forEach(({ i }) => used.add(i));

  if (!result.phone) {
    // 2. Fallback: a line that looks like a phone number (all digits/separators)
    const phoneOnlyIdx = lines.findIndex(
      (l, i) => !used.has(i) && /^\+?[\d\s\-().+]{8,}$/.test(l),
    );
    if (phoneOnlyIdx >= 0) {
      result.phone = lines[phoneOnlyIdx].trim();
      used.add(phoneOnlyIdx);
    } else {
      // 3. Last resort: Thai mobile / landline pattern anywhere in text
      const m = text.match(
        /(?:\+?66[-\s]?|0)[689]\d[-\s]?\d{3,4}[-\s]?\d{4}/,
      );
  ...
  // ── COMPANY NAME ───────────────────────────────────────────────────────────
  const companyPat =
    /co\.,?\s*ltd|co\.,?\s*limited|\binc\b\.?|\bcorp\b\.?|\bplc\b|\bcompany\b|\bgroup\b|\bindustries?\b|\binternational\b|\benterprise\b|\btrading\b|บริษัท|หจก\.|จำกัด|\bpty\b|\bgmbh\b|\bllc\b|\bs\.a\b/i;
